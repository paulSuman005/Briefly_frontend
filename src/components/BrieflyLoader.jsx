...
const BrieflyLoader = ({ size = 180, text = 'Loading...' }) => {
  ...
  return (
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#f8fafc] dark:bg-gray-900 p-4">
      ...
      {/* Moving Navy-Blue Border Wrapper */}
      <div className="relative rounded-3xl overflow-hidden p-[4px] max-w-3xl w-full shadow-xl">

        {/* Animated Gradient for the moving border */}
        <div className="absolute inset-0 bg-[conic-gradient(from_0deg,#0a1628,#1754d6,#0a1628,#1754d6,#0a1628)] animate-[spin_4s_linear_infinite]"></div>

        {/* Actual Card Content (White Background) */}
        <div className="relative bg-white rounded-3xl px-8 md:px-16 py-10 md:py-14 flex flex-col md:flex-row items-center justify-between w-full h-full z-10 gap-10 md:gap-0">
          ...
          <div className="flex flex-col items-center md:items-start text-center md:text-left gap-5 flex-shrink-0">
            ...
            <div className="flex items-center justify-center md:justify-start gap-3 mt-2 text-[#0a1628] text-xs uppercase tracking-widest font-semibold">
              <span className="loading loading-spinner loading-xs text-[#1754d6]"></span>
              <span>{text}</span>
            </div>
